        ...
        for gap in self.mid_gaps:
            ...
            if hull:
                ...
                self.draw(hull, idx, remain_idx, colormap.pop(0),
                          f'./images/第{i}壳层示意图.jpg')
                shell = pd.DataFrame(np.around(self.pos_matrix[idx, :], 4),
                                     columns=['x', 'y', 'z'])
                shell.insert(0, 'atom', [self.atoms[k] for k in idx])
                self.shell_and_pics.append((f'./images/第{i}壳层示意图.jpg', shell))
                self.log.info(msg=f'找到第{i}壳层,正在保存')
                i += 1


if __name__ == '__main__':
    # for f in os.listdir('./cif/')[:1]:
    # c = CrystalShell(filename='./cif/t-YSZ-Al10.cif')
    c = CrystalShell(filename='./test_cif_files/alpha_Nb5Si3.cif')
    print(c.shell_and_pics)